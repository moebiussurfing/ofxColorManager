...
//#endif
	...
	colorQuantizer.setImGuiPointer(ui.getGui());
	//ofxImGui::Gui gui = ui.getGuiPtr();
...
void ofxColorManager::gui_Theory()
{
	...
	ui.BeginWindow(SHOW_Theory, flags);
	//ui.BeginWindow("THEORY", flags);
	{
		ImGui::Dummy(ImVec2(0.0f, 5.0f));

		// amount colors
		//ImGui::PushItemWidth(110);
		if (ui.Add(numColors_Engines, OFX_IM_STEPPER)) {}
		...
		if (SHOW_Advanced)
		{
			...
			{
				ui.Add(bAuto_Theory_FromPicker);
				ui.Add(bAuto_Build_Palette);
				//ui.Add(analogSpread);
				//// layout
				//ImGui::Dummy(ImVec2(0.0f, 5.0f));
		...
		if (last_Index_Theory == 12) ui.Add(analogSpread);
		if (last_Index_Theory == 14) ui.Add(triadSpread);
...
void ofxColorManager::gui_Editor()
{
	...
	ui.BeginWindow(SHOW_Editor, flagsw);
	{
		float _spcx;
		float _spcy;
		float _w100;
...

		{
			...
			if (bEditPalette)
			{
				...
				if (ui.Add(last_Index_ColorPalette)) {
				};
				ImGui::PopItemWidth();
		...
		ofxImGuiSurfing::AddSpacingSeparated();
			...
			if (bSortPalette)
			{
...

				{
					...
					if (ofxImGuiSurfing::AddSmallButton(bFlipUserPalette, _w100, _h / 4)) {}
					//ImGui::SameLine();

					// sort random

					if (ImGui::Button("SORT RANDOM", ImVec2(_w100, _h / 4)))
					{
...
//#endif
			...
			if (SHOW_Advanced)
			{
				...
				//ui.Add(bAutoResizePalette);//not works
			}
...
void ofxColorManager::gui_Palette()
{
	...
	ui.BeginWindow(SHOW_Palette, flagsw);
	{
		ImGuiColorEditFlags _flags = ImGuiColorEditFlags_None;
...
void ofxColorManager::gui_Library()
{
	...
	ui.BeginWindow(SHOW_Library, flagsw);
	{
		bool bUpdate = false;
...
#endif
		{
			if (SHOW_Advanced)
			{
				if (ImGui::CollapsingHeader("Layout"))
				{
					...
					ui.Add(lib_Responsive_ModeFit);
					if (!lib_Responsive_ModeFit)
					{
						ui.Add(lib_Responsive_ModeGrid);
...
void ofxColorManager::gui_LinkExport()
{
	...
	ui.BeginWindow(SHOW_LinkExport, flagsw);
	{
		float _spcx;
		float _spcy;
		float _w100;
		...
		//if (ImGui::CollapsingHeader("Link"))
		{
			...
			////ofxImGuiSurfing::AddParameter(bAutoExportPreset);
			...
			if (ImGui::CollapsingHeader("EXTRA"))
			{
				if (bExportByFile)
					...
					{
						ofxImGuiSurfing::AddParameter(bExportPreset_DefaultPath);
...
void ofxColorManager::gui_Picker()
{
	...
	ui.BeginWindow(SHOW_Picker, flagsw);
	{
		float _spcx;
		float _spcy;
		float _w100;
		...
		if (ImGui::CollapsingHeader("HSB", _flagw))
		{
			...
			{
				//TODO:
				//if (ofxImGuiSurfing::AddParameter(color_HUE)) {}
				//if (ofxImGuiSurfing::AddParameter(color_SAT)) {}
				//if (ofxImGuiSurfing::AddParameter(color_BRG)) {}
		...
		// 3. LUV
		bool bOpen2 = false;
		ImGuiColorEditFlags _flagw2 = (bOpen2 ? ImGuiWindowFlags_NoCollapse : ImGuiWindowFlags_None);
		if (ImGui::CollapsingHeader("LUV", _flagw2))
		{
			ui.refreshLayout();

			bool bchanged = false;
			//ImGui::SameLine();
			if (ofxImGuiSurfing::AddParameter(color_Luv_Hue)) {
				bchanged = true;
			}
			if (ofxImGuiSurfing::AddParameter(color_Luv_Luminance)) {
				bchanged = true;
			}
			if (bchanged) {
				ofFloatColor c = HLuv.getColor(color_Luv_Hue, color_Luv_Luminance);
				color_Picked.set(c);
				//cTmp = ImVec4(c.r, c.g, c.b, 1.0f);
				//cTmp = color_Picked.get();
				////bChg_Pick = true;
			}
		}
		...
		if (ImGui::CollapsingHeader("RANDOMIZER"))
		{
			...
			{ if (ofxImGuiSurfing::AddParameter(bColor_rHUE)) {}ImGui::SameLine(); }
			{ if (ofxImGuiSurfing::AddParameter(bColor_rSAT)) {}ImGui::SameLine(); }
			{ if (ofxImGuiSurfing::AddParameter(bColor_rBRG)) {} }
			//ImGui::PopItemWidth();
			...
			if (bColor_rHUE)
				if (ofxImGuiSurfing::AddParameter(color_HUE_Power))
				{
					ofLogNotice(__FUNCTION__) << "HUE P : Moved";
				}

			if (bColor_rSAT)
				if (ofxImGuiSurfing::AddParameter(color_SAT_Power))
				{
					ofLogNotice(__FUNCTION__) << "SAT P : Moved";
				}

			if (bColor_rBRG)
				if (ofxImGuiSurfing::AddParameter(color_BRG_Power))
				{
		...
		if (bChg_Pick)
		{
			...
			color_Luv_Hue = ofMap(color_HUE.get(), 0, 255, 1, 360);
			//bFlag_refresh_EnginesFromPalette = true;
		}
...
void ofxColorManager::gui_EnginesPanel()
{
	...
	ui.BeginWindow(SHOW_Engines, flagsw);
	{
...
void ofxColorManager::gui_Advanced()
{
	...
	ui.BeginWindow(SHOW_Advanced, flags);
	{
		float _spcx;
		float _spcy;
		float _w100;
		...
		{
			ofxImGuiSurfing::AddParameter(SHOW_Advanced);
			ofxImGuiSurfing::AddParameter(SHOW_Layouts);
			ofxImGuiSurfing::AddParameter(bForceLayoutPosition);
			ofxImGuiSurfing::AddParameter(SHOW_Panels);
			ofxImGuiSurfing::AddParameter(SHOW_MINI_Preview);
			ofxImGuiSurfing::AddParameter(SHOW_MenuBar);
		...
		if (ImGui::CollapsingHeader("THEME", ImGuiWindowFlags_None))
		{
			...
			ofxImGuiSurfing::AddParameter(Lock_DockingLayout);
#ifdef USE_DEBUG_LAYOUT 
		...
		{
			//ofxImGuiSurfing::AddParameter(SHOW_Name);
			ofxImGuiSurfing::AddParameter(bAutoHide_Name);

			ImGui::PushItemWidth(85);
			ofxImGuiSurfing::AddStepper(fontSizeParam);
			ofxImGuiSurfing::AddStepper(fontSizeBigParam);
			ImGui::PopItemWidth();
		}
		...
		if (ImGui::CollapsingHeader("DEBUG", ImGuiWindowFlags_None))
		{
			...
			ofxImGuiSurfing::AddParameter(bDebugRectCentral);
...
void ofxColorManager::gui_LayoutsPresets()
{
	...
	ui.BeginWindow(SHOW_Layouts, flags);
	{
...
void ofxColorManager::gui_LayoutsAdvanced()
{
	...
	ui.BeginWindow(SHOW_LayoutsAdvanced, flags);
	{
		float _spcx;
		float _spcy;
		float _w100;
...
void ofxColorManager::gui_Panels()
{
	...
	ui.BeginWindow(SHOW_Panels, flags);
	{
...
void ofxColorManager::gui_Range()
{
	...
	ui.BeginWindow(SHOW_Range, flags);
	{
		...
		if (ui.Add(numColors_Range, OFX_IM_STEPPER)) {}
		//ImGui::PopItemWidth();
		...
		// 1. colors
		{
			...
			if (SHOW_Advanced)
			{
				if (ImGui::CollapsingHeader("Advanced"))
				{
					ofxImGuiSurfing::AddParameter(bAuto_Build_Palette);
					ofxImGuiSurfing::AddParameter(autoGenerate_Range);
					...
					if (ImGui::CollapsingHeader("Layout"))
					{
						...
						//ofxImGuiSurfing::AddParameter(scale_ColRange);
					}
				}
				ImGui::Dummy(ImVec2(0.0f, 5.0f));
			}
...
void ofxColorManager::gui_Background()
{
	...
	ui.BeginWindow(SHOW_BackGround, flagsw);
	{
		//--
...

		{
			...
			if (ImGui::CollapsingHeader("Advanced"))
			{
				...
				ofxImGuiSurfing::AddParameter(background_Draw_ENABLE);
				ofxImGuiSurfing::AddParameter(color_BackGround_Lock);

				if (!color_BackGround_Lock)
				{
					ofxImGuiSurfing::AddParameter(AutoSet_BackGround_Color);
					...
					if (AutoSet_BackGround_Color)
					{
						...
						{
							ofxImGuiSurfing::AddParameter(color_BackGround_AmtDarker);
						}
						if (color_BackGround_GradientMode)
						{
							//ImGui::Text("From Gradient:");
							ofxImGuiSurfing::AddParameter(gradientEngine.pickIn);
						}
					}
				}
				ofxImGuiSurfing::AddParameter(color_backGround_SET);
...
void ofxColorManager::gui_TextInput()
{
	...
	ui.BeginWindow("PRESET NAME", flagsw);
	//ui.BeginWindow("PRESET NAME");
	{
		ImGui::Dummy(ImVec2(0.0f, 5.0f));
...
void ofxColorManager::gui_Presets()
{
	...
	ui.BeginWindow(SHOW_Presets, flags);
	{
		float _spcx;
		float _spcy;
		float _w100;
		...
		if (ImGui::Button("RANDOM", ImVec2(_w100, _h / 2)))
		{
			preset_RandomIndex();
		}
		...
		if (ImGui::CollapsingHeader("EXTRA"))
		{
			...
			*/
			...
			//if (ofxImGuiSurfing::AddParameter(bPlaySlideShow)) {}
			ui.Add(bPlaySlideShow, OFX_IM_TOGGLE_BUTTON_ROUNDED_SMALL);
			//ImGui::PopItemWidth();

			//ImGui::PushItemWidth(-35);
			if (bPlaySlideShow) ui.Add(auto_pilot_Duration, OFX_IM_DEFAULT);
			//if (bPlaySlideShow) ofxImGuiSurfing::AddParameter(auto_pilot_Duration);
			ui.Add(bAutoHide_Name, OFX_IM_TOGGLE_BUTTON_ROUNDED_SMALL);
			//ofxImGuiSurfing::AddParameter(bAutoHide_Name);
			//ImGui::PopItemWidth();
		}
		...
		ui.Add(SHOW_Editor, OFX_IM_TOGGLE_BUTTON_ROUNDED_SMALL);
		ui.Add(SHOW_Kit, OFX_IM_TOGGLE_BUTTON_ROUNDED_SMALL);
		//ofxImGuiSurfing::AddBigToggle(SHOW_Editor, _w100, _h / 2);
		//ofxImGuiSurfing::AddBigToggle(SHOW_Kit, _w100, _h / 2);

		//ImGui::Dummy(ImVec2(0.0f, 2.0f));
		//if (SHOW_Kit) ofxImGuiSurfing::AddParameter(AutoScroll);
		//ImGui::Checkbox("Auto-Resize", &auto_resize);
	}
...
void ofxColorManager::gui_Gradient()
{
	...
	ui.BeginWindow(SHOW_Gradient, flagsw);
	{
		float _spcx;
		float _spcy;
		float _w100;
		...
		//if (ImGui::CollapsingHeader("EDIT", ImGuiWindowFlags_NoCollapse))
		{
			...
			ui.Add(gradientEngine.bGui_WidgetGradientTool, OFX_IM_TOGGLE_BUTTON_ROUNDED_SMALL);
			...
			ImGui::PushItemWidth(-40);
			ofxImGuiSurfing::AddParameter(gradientEngine.pickIn);
			ofxImGuiSurfing::AddParameter(gradientEngine.expTweak);
			ImGui::PopItemWidth();
			...
			{
				ofxImGuiSurfing::AddParameter(color_BackGround);
				ofxImGuiSurfing::AddParameter(gradientEngine.bAutoSet_PickColor);
			}
			...
			//ofxImGuiSurfing::AddParameter(gradientEngine.bAutoPaletteFromGradient);
		}
		...
		{
			ofxImGuiSurfing::AddParameter(gradientEngine.gradient_HardMode);
			...
			{
				ImGui::PushItemWidth(-40);
				ofxImGuiSurfing::AddParameter(gradientEngine.bTEST_Enable);
...
void ofxColorManager::gui_Demo()
{
	...
	ui.BeginWindow(SHOW_Demos, flagsw);
	{
		float _spcx;
		float _spcy;
		float _w100;
			...
			//if (ImGui::CollapsingHeader("DEMO 2", ImGuiWindowFlags_None))
		{
			if (DEMO2_Enable)
			{
				...
				_pad = -100;
				ImGui::PushItemWidth(_pad);
				ui.Add(DEMO2_Spheres.alpha_);
				ui.Add(DEMO2_Spheres.zoom);
				ui.Add(DEMO2_Spheres.speed);
				if (ui.Add(DEMO2_Spheres.mouseCam))
				{
		...
		// svg demo
		ofxImGuiSurfing::AddBigToggle(DEMO3_Svg.bEnable, _w100, _h);
			...
			//if (ImGui::CollapsingHeader("DEMO 3", ImGuiWindowFlags_None))
		{
			if (DEMO3_Svg.bEnable)
			{
				...
				ui.Add(DEMO3_Svg.bEdit);
				if (DEMO3_Svg.bEdit) ui.Add(DEMO3_Svg.scaleSvg);
				ui.Add(DEMO3_Svg.alphaSvg);
				ofxImGuiSurfing::AddStepper(DEMO3_Svg.blendMode);
				ui.Add(DEMO3_Svg.blendModeName);
				ofxImGuiSurfing::AddStepper(DEMO3_Svg.fileIndex);
				ui.Add(DEMO3_Svg.fileIndexName);
#ifdef USE_SVG_MASK
				ui.Add(DEMO3_Svg.enable_Mask);
				ui.Add(DEMO3_Svg.DEMO2_BgWhite);
#endif		
				ui.Add(DEMO3_Svg.bKeys);
...
bool ofxColorManager::draw_Gui()
{
	...
	//gui.begin();
	{
		...
		{
			{
				//ui.BeginWindow(SHOW_EditTheme, ImGuiWindowFlags_None);
				ui.BeginWindow("SHOW_EditTheme");
				{
			...
			ui.BeginWindow("STYLE SLECTOR");
			//ui.BeginWindow("STYLE SLECTOR", ImGuiWindowFlags_None);
			{
...
void ofxColorManager::Changed_Controls(ofAbstractParameter &e)
{
	...
	else if (name == fontSizeParam.getName())
	{
		...
		customFont = ui.getGui().addFont(_path + fontName, fontSizeParam, nullptr, normalCharRanges);
	...
	else if (name == fontSizeBigParam.getName())
	{
		...
		std::string _path = "assets/fonts/";//assets folder
		customFontBig = ui.getGui().addFont(_path + fontBigName, fontSizeBigParam, nullptr, normalCharRanges);
