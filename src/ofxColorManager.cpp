...
void ofxColorManager::update(ofEventArgs & args)
{
	...
	// DEMO1
	{
		if (DEMO1_Test) myDEMO1.update();

		if (DEMO1_Test && DEMO_Auto)
...
#endif
{
	...
	if (SHOW_Scene)
	{
		...
		{
			if (AutoSet_BackGround_Color.get())// && SHOW_GradientCurve.get()) 
			{
...
void ofxColorManager::gui_Palette()
{
	...
	if (ofxImGui::BeginWindow("PALETE", mainSettings, flagsw))
	{
...
void ofxColorManager::gui_Panels()
{
	...
	if (ofxImGui::BeginWindow("MAIN PANEL", mainSettings, flags))
	{
		...
		ofxSurfingHelpers::AddBigToggle(SHOW_Demo1);
	...
	if (ofxImGui::BeginWindow("PRESETS", mainSettings, flags))
	{
		...
		*/
		...
		if (ImGui::CollapsingHeader("Palette", _flagw))
		{
				...
				*/
				...
				ImGui::PushID(n);
				{
					...
					if ((n % _r) != 0)
					{
						...
						{
							ImGui::SameLine();
							//ImGui::SameLine(_spc,0);
							//ImGui::SameLine(0,0);
						}
					}

					// b. tweakable like palette panel
					//int _r = (int)MIN(boxMaxRows.get(), palette.size());
					//if (!bResponsive_Presets)
					//{
					//	//split in rows
			...
			//            ofLogNotice(__FUNCTION__) << "PRESET_Name: " << PRESET_Name;
			//
			//            PRESET_Temp.palette_load(PRESET_Name);
			//
			//            //-
			//
			//            color_BackGround = PRESET_Temp.getBackground();//TODO: temp solution because required pointer..
			//
			////            //TODO
