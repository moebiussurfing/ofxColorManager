...
// set positions and panel sizes
	...
	amountColors_Alg.set("Amount Colors", 6, 2, 10);
	...
	SHOW_ColorQuantizer.setName("PICTURE CAPTURE");
	...
	XML_params.add(color_HUE_RndMin);
	XML_params.add(color_SAT_RndMin);
	XML_params.add(color_BRG_RndMin);

	XML_params.add(color_HUE_RndMax);
	XML_params.add(color_SAT_RndMax);
	XML_params.add(color_BRG_RndMax);
...
#ifdef USE_IMAGE_QUANTIZER
	if (SHOW_ColorQuantizer) colorQuantizer.draw();
#endif
...
void ofxColorManager::gui_Theory()
{
	...
	{
		if (ofxImGui::AddParameter(primaryColorTheory))
		{
		}
...
void ofxColorManager::gui_Picker()
{
	if (ofxImGui::BeginWindow("PICKER", mainSettings, false))
	{
		...
		ImGui::ColorButton("MyColor##Picker", *(ImVec4 *)&color, misc_flags, ImVec2(_w, _h));
		...
		if (ImGui::CollapsingHeader("PALETTE"))
		{
			...
			ofxSurfingHelpers::AddBigButton(bAddColor, w50, 30);
			ImGui::SameLine();
...
void ofxColorManager::gui_Range()
{
	...
	if (ofxImGui::BeginWindow("RANGE", mainSettings, false))
	{
		...
		//picker
		ImGui::ColorPicker3("1", &guiCol1[0], misc_flags);
		...
		//misc_flags = ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoTooltip;
		//ImGui::ColorButton("MyColor##From", *(ImVec4 *)&guiCol1[0], misc_flags, ImVec2(w50, _h));
		...
		ImGuiColorEditFlags colorEdiFlags =
			...
			ImGuiColorEditFlags_NoAlpha |
			ImGuiColorEditFlags_PickerHueWheel;
		ImGui::ColorButton("MyColor##PickCircle", *(ImVec4 *)&guiCol1[0], colorEdiFlags, ImVec2(w50, _h));
		...
		ImGui::ColorPicker3("2", &guiCol2[0], misc_flags);

		//--

		// color big

		ImGui::ColorButton("MyColor##To", *(ImVec4 *)&guiCol2[1], misc_flags, ImVec2(w50, _h));
...
void ofxColorManager::refreshColorTheory()
{
	...
	//scheme->setPrimaryColor(primaryColorTheory.get());
	...
	for (int i = 0; i < NUM_COLOR_THEORY_TYPES; i++)
	{
		...
		_scheme->setPrimaryColor(primaryColorTheory.get());
...
void ofxColorManager::palettes_colorTheory_setup()
{
	...
	params_ColorTheory.add(primaryColorTheory.set("Base", ofColor::magenta, ofColor(0), ofColor(255)));
	...
	params_ColorTheory.add(amountColors.set("Amount Colors", 8, 2, 20));
...
void ofxColorManager::Changed_ColorTheory(ofAbstractParameter &e)
{
	...
	else if (name == colorScheme.getName() || name == primaryColorTheory.getName())
	{
		refreshColorTheory();
	}
...
void ofxColorManager::Changed_Controls(ofAbstractParameter &e)
{
	...
	{
		//if (SHOW_Presets && SHOW_ColorQuantizer)
		//    colorQuantizer.setActive(false);
		//else if (!SHOW_Presets && SHOW_ColorQuantizer)
		//    colorQuantizer.setActive(true);
	}
	...
	if (name == "RANDOM COLOR")
	{
		if (bRandomColor)
		{
			...
			//float _hue = ofRandom(color_HUE_RndMin/255.f, color_HUE_RndMax / 255.f);

			if (bColor_HUE) {
				_hue = ofRandom(color_HUE_RndMin, color_HUE_RndMax);
				color_HUE = _hue;
			}
			if (bColor_SAT) {
				_sat = ofRandom(color_SAT_RndMin, color_SAT_RndMax);
				color_SAT = _sat;
			}
			if (bColor_BRG) {
				_brg = ofRandom(color_BRG_RndMin, color_BRG_RndMax);
				color_BRG = _brg;
			}
...
void ofxColorManager::update_color_picked_CHANGES()
{
	...
	if (color1FromPicker) {
		...
		if (bRangeAutoGenerate) generateRange(col1range, col2range);
	}
