...
void ofApp::setup() {
	...
	ss << "MUST ENABLE SOME CONVERTER BATCH-SCRIPT BELOW\n-> ofApp::setup()";
...
void ofApp::batch4() {
		...
		*/
	{
		...
		string path = "OUTPUT/" + kitName; // set kit folder name
		string filenameRoot = "SANZO_WADA"; // set palette names root
		...
		if (file.exists())
		{
			...
			for (int p = 0; p < icombinations.size(); p++)
			{
				...
				// 2.2 store json if valid (= there's one or more colors)
			...
			// save json
			string filename = filenameRoot + "_Library.json";
			ofSavePrettyJson("OUTPUT/" + filename, jp);
		}
	}
}

//TODO:
// this is only a color library,
// we don't need to convert palettes format.
// we will use this function only if we require conversion to import
// colors into ofxColorBrowser...
//--------------------------------------------------------------
void ofApp::batch5() {

	//// Taken from https://github.com/cheprasov/json-colors
	///*
	//	[
	//  {
	//	"name": "Absolute Zero",
	...
	//{
	//	// set the source file to convert
	//	ofFile file("cheprasov-colors.json");

	//	string kitName = "CHEPRASOV KIT";
	//	string path = "OUTPUT/" + kitName; // set kit folder name
	//	string filenameRoot = "Cheprasov"; // set palette names root
	...
	//				if (bDebug) {
	//					ss << "color #" << ic++ << " " << _hex << endl;
	//				}

	//				ofStringReplace(_hex, "#", ""); // remove "#"
	//				ofColor myColor;
	//				hexToColor(myColor, _hex);
	...
	//			// save json
	//			ofSavePrettyJson(path + "/" + filename, ic);
	//		}
	//		ss << "Total processed Colors: " << ic << endl;
	//	}
	//}
}
