...
ofxGuiPanelsLayout::ofxGuiPanelsLayout()
{
	...
	ofxSurfingHelpers::CheckFolder(pathFolder);
	...
	SHOW_draggable.set("Show Draggables", false);
...
#ifdef USE_GUI_TYPE_TEXT_FLOW
//--------------------------------------------------------------------------
void ofxGuiPanelsLayout::TextFlow_setTitle(std::string _title)
{
...
void ofxGuiPanelsLayout::setup()
{
	...
	ofLogVerbose(__FUNCTION__) << "size_OFX_GUI " << size_OFX_GUI;
#endif
#ifdef USE_GUI_TYPE_OFX_DAT_GUI
	size_OFX_DAT = guis_OFX_DAT_GUI.size();
	ofLogVerbose(__FUNCTION__) << "size_OFX_DAT " << size_OFX_DAT;
#endif
#ifdef USE_GUI_TYPE_TOGGLE_PARAM
	size_TOGGLES = guis_TOGGLE.size();
	ofLogVerbose(__FUNCTION__) << "size_TOGGLES " << size_TOGGLES;
#endif
#ifdef USE_GUI_TYPE_TEXT_BOX
	size_OFX_TEXT_BOX = guis_OFX_TEXT_BOX.size();
	ofLogVerbose(__FUNCTION__) << "size_OFX_TEXT_BOX " << size_OFX_TEXT_BOX;
#endif
...
#endif
	ofLogVerbose(__FUNCTION__) << "size_ALL_PANELS " << size_ALL_PANELS;
...
#endif
	ofLogVerbose(__FUNCTION__) << "size_ALL_GUIS " << size_ALL_GUIS;
...
void ofxGuiPanelsLayout::draw()
{
	...
	if (SHOW_controlPanel) gui_Control.draw();
	if (SHOW_advanced) gui_Control_Advanced.draw();
	if (SHOW_draggable) controlPoints.draw();
...
void ofxGuiPanelsLayout::Changed_parameters(ofAbstractParameter &e)
{
	...
	//    ofLogNotice(__FUNCTION__) << "Changed_parameters: " << name << ": " << e;
	...
	for (int p = 0; p < panels.size(); p++)
	{
		if (name == panels[p].name.get())
		{
			ofLogVerbose(__FUNCTION__) << "PANEL: " << name << ": " << e;

			//            if (name == "DEBUG CAMERAS")
			//            {
			//                ofLogVerbose(__FUNCTION__) << "+ DEBUG CAMERAS DETECTED";
			...
			{
				int first = i - start;
				if (name == guis_TOGGLE[first]->getName())
				{
					//ofLogVerbose(__FUNCTION__) << "+ TOGGLE_" << ofToString(first) << " DETECTED";
					if (panels[i].state)
					{
...
void ofxGuiPanelsLayout::Changed_parametersAdvance(ofAbstractParameter &e)
{
	...
	else if (name == "APP MODE" && (group_Selected_PRE != group_Selected))
	{
		if (group_Selected < 0)
			group_Selected = 0;
	...
	else if (name == "Hide All" && group_hideAll)
	{
		for (int i = 0; i < panels.size(); i++)
		{
			group_hideAll = false;
...
void ofxGuiPanelsLayout::loadPanels()
{
	...
	{
		file >> js;
		ofLogVerbose(__FUNCTION__) << js;
		ofLogVerbose(__FUNCTION__) << endl << "-----" << endl;
		...
		for (auto &position : js)
		{
			if (!position.empty() && (i < panels.size()))
			{
				...
				ofLogVerbose(__FUNCTION__) << "PANEL:" << name << " x:" << x << " y:" << y << endl;
	...
	{
		ofLogError(__FUNCTION__) << "JSON file not found";
	}
	...
	if (file2.exists())
	{
		...
		ofLogNotice(__FUNCTION__) << "group_Selected:" << group_Selected;
...
{
	if (group_autoSave) group_Save = true;
